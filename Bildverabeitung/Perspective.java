    ...
    void calculateNearestNeigbour(int srcPixels[], int srcWidth, int srcHeight, int dstPixels[], int dstWidth, int dstHeight, double degrees) {
        ...
        for (int y = 0; y < dstHeight; y++) {
            for (int x = 0; x < dstWidth; x++) {
                ...
                int xCenter = x - dstWidth/2;
                int yCenter = y - dstHeight/2;

                double srcY = yCenter / (cos - s * sin * yCenter);
                double srcX = xCenter * (s * sin * srcY + 1);
              
                int xNew = (int) srcX + srcWidth/2;
                int yNew = (int) srcY + srcHeight/2;
...
//                int dstPos = y * dstWidth + x;
//
//                ///// Center the image in the available canvas space /////
//                int xMid = x - (dstWidth / 2);
//                int yMid = y - (dstHeight  / 2);
//
//                ///// Get source location based on angle /////
//                double srcY = yMid / (cos - s * sin * yMid);
//                double srcX = xMid * (s * sin * srcY + 1);
//
//                int xOut = (int) srcX + (srcWidth  / 2);
//                int yOut = (int) srcY + (srcHeight / 2);
//
//                int srcPos = yOut * srcWidth + xOut;
//
//                ////// If outside of image -> draw white pixels /////
//                if(yOut < srcHeight && xOut < srcWidth && yOut >= 0 && xOut >= 0){
